...
define(['app', 'echarts', './theme'], function (app, echarts, barChart) {
    var types = ['line', 'bar', 'pie', 'scatter'];
    for (var i = 0, n = types.length; i < n; i++) {
        (function (type) {
            app.register.directive(type + 'Chart', ['$http', 'theme', function ($http, theme) {
                return {
                    restrict: 'EA',
                ...
                function getLinkFunction(type, theme) {
                    ...
                    return function (scope, element, attrs) {
                        ...
                        scope.$watch('options', function () {
                            ...
                            if (options) {
                                var themeValue=theme.get(scope.theme.color || 'blue');
                                themeValue.textStyle.fontFamily=scope.theme.textStyle.fontFamily;
                                if (options.needInit) {
                                    if (myChart && myChart.dispose) {
                                        myChart.dispose();
                                    }
                                    myChart = echarts.init(element[0], themeValue);
                                } else {
                                    if (!myChart) {
                                        myChart = echarts.init(element[0], themeValue);
                                    }
