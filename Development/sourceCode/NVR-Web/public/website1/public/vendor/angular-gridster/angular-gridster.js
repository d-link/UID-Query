		...
		function(gridsterConfig, $timeout) {
			...
			this.updateHeight = function(plus) {
				...
				if(this.grid.length>0){
					this.gridWidth=this.grid[0].length*(this.colWidth);
				}
				...
				compile: function($tplElem) {
					...
					return function(scope, $elem, attrs, gridster) {
						...
						function updateWidth(){
							if(gridster.curWidth<gridster.gridWidth){
								gridster.curWidth=gridster.gridWidth;
								$elem.css('width', (gridster.curWidth ) +1+ 'px');
							}
						}
			...
			function ResizeHandle(handleClass) {
				...
				function mouseMove(e) {
					...
					/*if (gridster.draggable.enabled !== savedDraggable) {
						...
						scope.$broadcast('gridster-resizing', {width:elmW,height:elmH});
					}*/
					return true;
				}
