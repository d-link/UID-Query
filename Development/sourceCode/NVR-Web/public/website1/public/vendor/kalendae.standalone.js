...
Kalendae.prototype = {
	...
	addSelected : function (date, draw) {
		...
		// Custom code for Dview 8. Use: Stop sort for check the date. Operation: Notes.
...
Kalendae.Input = function (targetElement, options) {
	...
	// Custom code for Dview 8. Use: for use textarea tag. Operation: Notes.
	...
	util.addEvent($container, 'mousedown', function (event, target) {
		...
		/* 
		 * Custom code for Dview 8.
		 * Use: Close the calendar when user choice one Date.
		 * Operation: Add.
		 */
		if (util.hasClassName(target.parentNode, classes.days) && util.hasClassName(target, classes.dayActive) && opts.mode=='single') {
			$input.blur();
		}
	});
	...
	util.addEvent($input, 'focus', function () {
		// self.setSelected(this.value); // Custom code for Dview 8.  Use: Cancellation of automatic assignment when user input  Operation: Notes.
		self.show();
		self.btn = false; // Custom code for Dview 8. Use: Record current status. Operation: Add.
	});
	...
	util.addEvent($input, 'blur', function () {
		...
		setTimeout(function(){ self.btn = true },200); // Custom code for Dview 8. Use: Record current status. Operation: Add.
	});
	/* 
	 * Custom code for Dview 8.
	 * Use: Cancellation of automatic assignment when user input
	 * Operation: Notes.
	 */
	...
	/* 
	 * Custom code for Dview 8.
	 * Use: Binging event with custom button.
	 * Operation: Add.
	 */
	if (opts.customBtnId) {
		var $btn = util.$(opts.customBtnId);
		self.btn = true;
		util.addEvent($btn, 'click', function () {
			if (self.btn&&document.activeElement.id!=targetElement) self.input.focus();
		});
	}
